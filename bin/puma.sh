...
case "$1" in
  start)
    ...
    if [ -e $PUMA_CONFIG_FILE ] ; then
      su shalwres <<'EOF'
cd /var/www/alwaysresolve.com/web/current && RAILS_ENV=production ~/.rvm/bin/rvm jruby-1.7.9@alwaysresolve do bundle exec puma --config /var/www/alwaysresolve.com/web/current/config/puma.rb
EOF
    else
      su shalwres <<'EOF'
